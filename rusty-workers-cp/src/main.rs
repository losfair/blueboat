...
use anyhow::Result;
use hyper::{Request, Response, Body, StatusCode, header::HeaderValue, service::{make_service_fn, service_fn}};
use std::sync::Arc;
    ...
    async fn handle(self: Arc<Self>, request: Request<Body>) -> Result<Response<Body>> {
        ...
        match req_path.as_str() {
            ...
            "/v1/add_route" => {
                ...
                self.kv.route_mapping_insert(&opt.domain, &opt.path, opt.appid).await?;
                Ok(mk_json_response(&())?)
            }
            "/v1/delete_domain" => {
...
async fn main() -> Result<()> {
    ...
    hyper::Server::bind(&opt.http_listen).serve(make_svc).await?;
    Ok(())
}
